...
To use Storyblok Management API, all paths in axios was prefixed with `/storyblok/`. If you want to get all stories from space ide 606, you would call with management API `spaces/606/stories/` here you call `/storyblok/spaces/606/stories/`.
...
export default {
  ...
  methods: {
    loadStories() {
      ...
      axios.get(`/storyblok/spaces/${this.$route.query.space_id}/stories`)
        .then((res) => {
...
export default {
  ...
  methods: {
    createStory() {
      ...
      // get the space id from URL and use it in requests
      return axios
        .post(`/storyblok/spaces/${this.$route.query.space_id}/stories`, body)
        .then((res) => {
          this.loading = false
        })
      }
...
To get information about the authenticated user, you should make a `GET` request to `/storyblok/user` path.
...
export default {
  ...
  methods: {
    loadUserInformation() {
      axios.get(`/storyblok/user?space_id=${this.$route.query.space_id}`)
        .then((res) => {
          this.user = res.data || {}
        })
      }
