...
const getRouter = (options) => {
  ...
  router.get('/explore/:space_id/*', async function (req, res, next) {
  ...
  router.post('/explore/:space_id/*', async function (req, res, next) {
    const client = getStoryblokClient(req.session, req.params.space_id, options)
    const body = req.body
    const url = getExploreUrl(req.url || '')
  ...
  router.put('/explore/:space_id/*', async function (req, res, next) {
    const client = getStoryblokClient(req.session, req.params.space_id, options)
    const body = req.body
    const url = getExploreUrl(req.url || '')
  ...
  router.delete('/explore/:space_id/*', async function (req, res, next) {
    const client = getStoryblokClient(req.session, req.params.space_id, options)
    const url = getExploreUrl(req.url || '')
